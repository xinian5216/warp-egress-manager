...
WORK_DIR="$(mktemp -d /tmp/warp3xui-install.XXXXXX)"
CURL_CONFIG="${WORK_DIR}/curl.conf"
SCRIPT_FILE="${WORK_DIR}/warp-3xui.sh"
CHECKSUM_FILE="${WORK_DIR}/warp-3xui.sha256"
...
curl --config "${CURL_CONFIG}" \
    ...
    -o "${SCRIPT_FILE}"
curl --config "${CURL_CONFIG}" \
    "${BASE_URL}/releases/warp3xui/warp-3xui.sha256" \
    -o "${CHECKSUM_FILE}"
