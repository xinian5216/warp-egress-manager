...
- Worker 保持现有 `latest/cloudflare-warp.deb` URL 不变，在内部安全读取版本指针并返回对应归档；指针缺失或迁移未完成时仍可回退旧对象。
- 同步工作流确认带有 `archive-pointer-v1` 标识的新版 Worker 已上线后，精确删除 5 个系统代号下旧的 `latest` 重复包；若 Worker 尚未部署则暂缓清理，由后续计划任务完成。
- 每个系统代号继续保留当前版与前一版两个归档，预期稳定存储由最多 3 份包降为 2 份，首次迁移后由 2 份降为 1 份。
